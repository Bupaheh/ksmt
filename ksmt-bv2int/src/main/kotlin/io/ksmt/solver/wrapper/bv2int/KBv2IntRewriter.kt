...
open class KBv2IntRewriter(
    ...
    val signedness = when (config.signednessMode) {
        SignednessMode.UNSIGNED -> UNSIGNED
        else -> SIGNED
    }

    val defaultWrapMode = when (signedness) {
    ...
    fun rewriteBv2Int(expr: KExpr<KBoolSort>): KExpr<KBoolSort> = with(ctx) {
        if (config.disableRewriting) return mkAndNoSimplify(ctx.trueExpr, expr)
        ...
        val lemma = mkAndNoSimplify(lemmaFlatten(transformedExpr.getLemma()))
    ...
    fun bvAndLemmas(expr: KExpr<KBoolSort>): List<KExpr<KBoolSort>> =
        if (!config.disableRewriting) lemmaFlatten(expr.getBvAndLemma()) else emptyList()

    fun overflowLemmas(expr: KExpr<KBoolSort>): KExpr<KBoolSort> =
        if (!config.disableRewriting) ctx.mkAndNoSimplify(lemmaFlatten(expr.getOverflowLemma())) else ctx.trueExpr

    private fun rewriteDecl(decl: KDecl<*>): KDecl<KSort> = with(ctx) {
        mkFuncDecl(
            decl.name,
    ...
    private fun <T : KSort> KExpr<T>.addLemma(lemma: KExpr<KBoolSort>): KExpr<T> =
        addLemma(toLemma(lemma))

    private fun <T : KSort> KExpr<T>.addLemma(lemma: Bv2IntLemma): KExpr<T> = apply {
        if (isEmptyLemma(lemma) || this is KInterpretedValue) return@apply
        val mergedLemma = mergeLemmas(lemma, getLemma())
        lemmas[tryUnwrap()] = mergedLemma
    }

    private fun <T : KSort> KExpr<T>.addBvAndLemma(lemma: KExpr<KBoolSort>): KExpr<T> =
        addBvAndLemma(toLemma(lemma))

    private fun <T : KSort> KExpr<T>.addBvAndLemma(lemma: Bv2IntLemma): KExpr<T> = apply {
        if (isEmptyLemma(lemma) || !config.isLazyBvAnd || this is KInterpretedValue) return@apply
        val mergedLemma = mergeLemmas(lemma, getBvAndLemma())
        bvAndLemmas[tryUnwrap()] = mergedLemma
    }

    private fun <T : KSort> KExpr<T>.addOverflowLemma(lemma: KExpr<KBoolSort>): KExpr<T> =
        addOverflowLemma(toLemma(lemma))

    private fun <T : KSort> KExpr<T>.addOverflowLemma(lemma: Bv2IntLemma): KExpr<T> = apply {
        if (isEmptyLemma(lemma) || !config.isLazyOverflow || this is KInterpretedValue) return@apply
        val mergedLemma = mergeLemmas(lemma, getOverflowLemma())
        overflowLemmas[tryUnwrap()] = mergedLemma
    }

    private fun KExpr<*>.getLemma() = lemmas.getOrDefault(tryUnwrap(), EMPTY_LEMMA)

    private fun KExpr<*>.getBvAndLemma() =
        if (config.isLazyBvAnd) {
            bvAndLemmas.getOrDefault(tryUnwrap(), EMPTY_LEMMA)
        } else {
            EMPTY_LEMMA
        }
    ...
    private fun KExpr<*>.getOverflowLemma() =
        if (config.isLazyOverflow) {
            overflowLemmas.getOrDefault(tryUnwrap(), EMPTY_LEMMA)
        } else {
            EMPTY_LEMMA
        }


    private fun shouldTransform(expr: KExpr<*>) = if (config.enableSplitter) !dsu.isMarked(expr) else true
    ...
    private inline fun rewriteEqZeroLshr(
        ...
        val shift = when {
            rhs is KBv2IntAuxExprLshr && lhs.denormalized == bv2IntContext.zero -> rhs
            lhs is KBv2IntAuxExprLshr && rhs.denormalized == bv2IntContext.zero -> lhs
            else -> return cont()
        }

        shift.originalExpr.normalized(UNSIGNED) le mkPowerOfTwoExpr(shift.shift.toUInt())
    }
    ...
    private fun <T : KSort> rewriteEq(lhs: KExpr<T>, rhs: KExpr<T>): KExpr<KBoolSort> = with(ctx) {
        ...
        val signedness = when {
            lhs.isNormalizedUnsigned && rhs.isNormalizedUnsigned -> UNSIGNED
            lhs.isNormalizedSigned && rhs.isNormalizedSigned -> SIGNED
            lhs.isNormalizedUnsigned -> UNSIGNED
            else -> SIGNED
        }
    ...
    override fun <T : KSort> transform(expr: KIteExpr<T>): KExpr<T> = with(ctx) {
        transformExprAfterTransformedBv2Int(
            ...
            val signedness = when {
                tBranch.isNormalizedSigned && fBranch.isNormalizedSigned -> SIGNED
                tBranch.isNormalizedUnsigned && fBranch.isNormalizedUnsigned -> UNSIGNED
                tBranch.isNormalizedSigned -> SIGNED
                else -> UNSIGNED
            }
    ...
    override fun transform(expr: KBitVec1Value): KExpr<KBv1Sort> = with(ctx) {
        transformExprAfterTransformedBv2Int(expr) {
            KBv2IntAuxExprConst(mkIntNum(if (expr.value) 1L else 0L), 1u, UNSIGNED)
        }
    }

    override fun transform(expr: KBitVec8Value): KExpr<KBv8Sort> = with(ctx) {
        transformExprAfterTransformedBv2Int(expr) {
            KBv2IntAuxExprConst(expr.numberValue.toUByte().toLong().expr, 8u, UNSIGNED)
        }
    }

    override fun transform(expr: KBitVec16Value): KExpr<KBv16Sort> = with(ctx) {
        transformExprAfterTransformedBv2Int(expr) {
            KBv2IntAuxExprConst(expr.numberValue.toUShort().toLong().expr, 16u, UNSIGNED)
        }
    }

    override fun transform(expr: KBitVec32Value): KExpr<KBv32Sort> = with(ctx) {
        transformExprAfterTransformedBv2Int(expr) {
            KBv2IntAuxExprConst(expr.numberValue.toUInt().toLong().expr, 32u, UNSIGNED)
        }
    }
    ...
    override fun transform(expr: KBitVecCustomValue): KExpr<KBvSort> = with(ctx) {
        transformExprAfterTransformedBv2Int(expr) {
            KBv2IntAuxExprConst(expr.value.normalizeValue(expr.sizeBits).expr, expr.sizeBits, UNSIGNED)
        }
    }
            ...
            when {
                canNormalize(arg) -> {
                    val result = if (arg.isNormalizedUnsigned) {
                        mkPowerOfTwoExpr(sizeBits) - bv2IntContext.one - arg.normalized(UNSIGNED)
                    } else {
                        val signedArg = arg.normalized(SIGNED)

                        mkIte(
                            condition = signedArg ge bv2IntContext.zero,
                            trueBranch = mkPowerOfTwoExpr(sizeBits) - bv2IntContext.one - arg.normalized(SIGNED),
    ...
    override fun <T : KBvSort> transform(expr: KBvReductionAndExpr<T>): KExpr<KBv1Sort> = with(ctx) {
        this@KBv2IntRewriter.transformExprAfterTransformedBv2Int(expr, expr.value) { arg: KExpr<KIntSort> ->
            ...
            mkIte(
                ...
                trueBranch = toSignedness(bv2IntContext.one, sizeBits, UNSIGNED),
    ...
    override fun <T : KBvSort> transform(expr: KBvReductionOrExpr<T>): KExpr<KBv1Sort> = with(ctx) {
        this@KBv2IntRewriter.transformExprAfterTransformedBv2Int(expr, expr.value) { arg: KExpr<KIntSort> ->
            ...
            mkIte(
                ...
                falseBranch = toSignedness(bv2IntContext.one, expr.sort.sizeBits, UNSIGNED)
            )
        }
    }

    override fun <T : KBvSort> transform(expr: KBvAndExpr<T>): KExpr<T> {
        require(!config.enableSplitter)

        return when (config.andRewriteMode) {
    ...
    private fun <T : KBvSort> transformBvAndSum(expr: KBvAndExpr<T>): KExpr<T> = with(ctx) {
        transformExprAfterTransformedBv2Int(
            ...
            when {
                config.rewriteMode == RewriteMode.EAGER || result is KInterpretedValue -> result
                config.rewriteMode == RewriteMode.LAZY -> {
                    val normalizedSignedness = UNSIGNED
    ...
    private fun <T : KBvSort> transformBvAndBitwise(expr: KBvAndExpr<T>): KExpr<T> = with(ctx) {
        transformExprAfterTransformedBv2Int(
            ...
            val normalizedArg1 = arg1.normalized(UNSIGNED)
            val result = if (config.rewriteMode == RewriteMode.EAGER) {
            ...
            result.tryAddBoundLemmas(expr.sort, UNSIGNED)
        }
    }
    ...
    private fun rewriteLia(
        ...
        when {
            config.signednessMode == SignednessMode.SIGNED && canNormalize ->
                KBv2IntAuxExprSingleOverflow(normalizedSignednessArgsValue, denormalizedValue, sizeBits, signedness)

            config.isLazyOverflow && canNormalize -> KBv2IntAuxExprNormalized(
    ...
    private fun rewriteBvMulExpr(
        ...
        return if (config.isLazyOverflow && canNormalize(arg0) && canNormalize(arg1)) {
            KBv2IntAuxExprNormalized(
...
//        KBv2IntAuxExprSingleOverflow(result, arg0.denormalized * arg1.denormalized, sizeBits)
    ...
    override fun <T : KBvSort> transform(expr: KBvSignedDivExpr<T>): KExpr<T> = with(ctx) {
        transformExprAfterTransformedBv2Int(
            ...
            val signedRes = mkIte(
                condition = mkAnd(
                    arg0 eq toSigned(mkPowerOfTwoExpr(sizeBits - 1u), sizeBits, UNSIGNED),
                    arg1 eq toSigned(mkPowerOfTwoExpr(sizeBits) - bv2IntContext.one, sizeBits, UNSIGNED)
                ),
                trueBranch = arg0,
                falseBranch = mkIte(
            ...
            mkIte(
                ...
                trueBranch = mkIte(
                    ...
                    trueBranch = toSigned(mkPowerOfTwoExpr(expr.sort.sizeBits) - bv2IntContext.one, sizeBits, UNSIGNED),
                    falseBranch = toSigned(bv2IntContext.one, sizeBits, UNSIGNED)
                ),
    ...
    private fun rewriteUle(arg0: KBv2IntAuxExpr, arg1: KBv2IntAuxExpr): KExpr<KBoolSort> = with(ctx) {
        rewriteUleConstLshr(arg0, arg1) {
            arg0.normalized(UNSIGNED) le arg1.normalized(UNSIGNED)
        }
    }
...
//    }
    ...
    private fun KContext.tryRewriteSleConstAshr(arg0: KBv2IntAuxExpr, arg1: KBv2IntAuxExpr): KExpr<KBoolSort>? {
        val arg0Normalized = arg0.normalized(SIGNED)
        val arg1Normalized = arg1.normalized(SIGNED)

        return when {
            arg0Normalized is KIntNumExpr && arg1 is KBv2IntAuxExprAshr -> mkArithLe(
                arg0Normalized * mkPowerOfTwoExpr(arg1.shift.toUInt()),
                arg1.originalExpr.normalized(SIGNED)
            )

            arg0 is KBv2IntAuxExprAshr && arg1Normalized is KIntNumExpr -> mkArithLt(
                arg0.originalExpr.normalized(SIGNED),
                (arg1Normalized + bv2IntContext.one) * mkPowerOfTwoExpr(arg0.shift.toUInt())
            )
    ...
    private inline fun KContext.rewriteSleSingleOverflow(
        ...
        when {
            ...
            arg0 is KBv2IntAuxExprSingleOverflow ->
                arg0.normalizedOp { arg -> arg le arg1.normalized(SIGNED) }

            arg1 is KBv2IntAuxExprSingleOverflow ->
                arg1.normalizedOp { arg -> arg0.normalized(SIGNED) le arg }
        ...
        rewriteSleConstLshr(arg0, arg1) {
            rewriteSleConstAshr(arg0, arg1) {
                rewriteSleSingleOverflow(arg0, arg1) {
                    arg0.normalized(SIGNED) le arg1.normalized(SIGNED)
                }
            }
        }
    }
    ...
    override fun transform(expr: KBvConcatExpr): KExpr<KBvSort> = with(ctx) {
        transformExprAfterTransformedBv2Int(
            ...
            val lowerBits = arg1.normalized(UNSIGNED)
            val normalizedResultSignedness = if (arg0.isNormalizedSigned) SIGNED else UNSIGNED
    ...
    override fun transform(expr: KBvExtractExpr): KExpr<KBvSort> = with(ctx) {
        this@KBv2IntRewriter.transformExprAfterTransformedBv2Int(
            ...
            if (config.isLazyOverflow && canNormalize(value)) {
                val normalizedSignedness = if (value.isNormalizedSigned) SIGNED else UNSIGNED
                ...
                KBv2IntAuxExprExtract(
                    ...
                    normalized =  normalizeExpr(denormalizedResult, UNSIGNED, sizeBits),
                    originalExpr = value.denormalized,
                    low = expr.low.toUInt(),
                    sizeBits = expr.sort.sizeBits,
                    normalizedSignedness = UNSIGNED,
                    canNormalize = false
                )
            }
        }
    ...
    override fun transform(expr: KBvRepeatExpr): KExpr<KBvSort> = with(ctx) {
        this@KBv2IntRewriter.transformExprAfterTransformedBv2Int(expr, expr.value) { value: KExpr<KIntSort> ->
            ...
            toSignedness(currentValue, expr.sort.sizeBits, UNSIGNED)
        }
    }

    private fun KContext.mkShiftCondition(value: KBv2IntAuxExpr, sizeBits: UInt) =
        if (value.isNormalizedUnsigned) {
            value.normalized(UNSIGNED) ge sizeBits.toLong().expr
        } else {
            val signedValue = value.normalized(SIGNED)
    ...
    private fun shiftRewriteCondition(shift: KBv2IntAuxExpr, sizeBits: UInt): Boolean {
        val shiftValue = shift.normalized(UNSIGNED)
    ...
    override fun <T : KBvSort> transform(expr: KBvShiftLeftExpr<T>): KExpr<T> = with(ctx) {
        transformExprAfterTransformedBv2Int(
            ...
            if (shift.normalized(UNSIGNED) == bv2IntContext.zero) return@transformExprAfterTransformedBv2Int arg

//            if (arg.denormalized is KConst) return@transformExprAfterTransformedBv2Int arg

            val sizeBits = expr.sort.sizeBits
            val normalizedShift = shift.normalized(UNSIGNED)
            ...
            val higherBits = if (config.isLazyOverflow && canNormalize(arg)) {
            ...
            when {
                shiftRewriteCondition(shift, sizeBits) -> {
                    ...
                    val normalized = if (config.isLazyOverflow && canNormalize(arg)) {
                        canNormalize = true
                        result
                    } else {
    ...
    override fun <T : KBvSort> transform(expr: KBvLogicalShiftRightExpr<T>): KExpr<T> = with(ctx) {
        transformExprAfterTransformedBv2Int(
            ...
            val result = mkIte(
                condition = normalizedShift ge sizeBits.toLong().expr,
                trueBranch = bv2IntContext.zero,
                falseBranch = arg.normalized(UNSIGNED) / bv2IntContext.mkPowerOfTwoApp(normalizedShift),
            )
            ...
            when {
                shiftRewriteCondition(shift, sizeBits) -> {
                    ...
                    if (arg is KBv2IntAuxExprZeroExtension && sizeBits - shiftNum <= arg.extensionSize) {
                        KBv2IntAuxExprConst(bv2IntContext.zero, sizeBits, UNSIGNED)
                    } else {
                        KBv2IntAuxExprLshr(
                            normalized = result,
                            originalExpr = arg,
        ...
        transformExprAfterTransformedBv2Int(
            ...
            val result = mkIte(
                ...
                falseBranch = if (shift.isNormalizedUnsigned) {
                    signedArg / bv2IntContext.mkPowerOfTwoApp(shift.normalized(UNSIGNED))
                } else {
                    signedArg / bv2IntContext.mkPowerOfTwoApp(shift.normalized(SIGNED))
                }
            )
            ...
            when {
                shiftRewriteCondition(shift, sizeBits) -> {
                    KBv2IntAuxExprAshr(
                        ...
                        shift = shift.normalized(UNSIGNED)
...
//            higherBits + lowerBits
    ...
    override fun transform(expr: KBv2IntExpr): KExpr<KIntSort> = with(ctx) {
        transformExprAfterTransformedBv2Int(
            ...
            when (expr.isSigned) {
                true -> arg.normalized(SIGNED)
                false -> arg.normalized(UNSIGNED)
            }
        }
    }
    ...
    override fun <T : KBvSort> transform(expr: KBvAddNoOverflowExpr<T>): KExpr<KBoolSort> = with(ctx) {
        transformExprAfterTransformedBv2Int(
            ...
            expr.arg0,
            expr.arg1,
            preprocessMode = if (expr.isSigned) NORMALIZED_SIGNED else NORMALIZED_UNSIGNED,
        ) { arg0: KExpr<KIntSort>, arg1: KExpr<KIntSort> ->
            val (_, upperBound) = getBounds(expr.arg0.sort.sizeBits, if (expr.isSigned) SIGNED else UNSIGNED)

            arg0 + arg1 le upperBound
        }
    }
    ...
    override fun <T : KBvSort> transform(expr: KBvAddNoUnderflowExpr<T>): KExpr<KBoolSort> = with(ctx) {
        transformExprAfterTransformedBv2Int(
            ...
            val (lowerBound, _) = getBounds(expr.arg0.sort.sizeBits, SIGNED)

            arg0 + arg1 ge lowerBound
        }
    }
    ...
    override fun <T : KBvSort> transform(expr: KBvSubNoOverflowExpr<T>): KExpr<KBoolSort> = with(ctx) {
        transformExprAfterTransformedBv2Int(
            ...
            val (_, upperBound) = getBounds(expr.arg0.sort.sizeBits, SIGNED)
    ...
    override fun <T : KBvSort> transform(expr: KBvSubNoUnderflowExpr<T>): KExpr<KBoolSort> = with(ctx) {
        transformExprAfterTransformedBv2Int(
            ...
            val (lowerBound, _) = getBounds(expr.arg0.sort.sizeBits, if (expr.isSigned) SIGNED else UNSIGNED)
    ...
    override fun <T : KBvSort> transform(expr: KBvDivNoOverflowExpr<T>): KExpr<KBoolSort> = with(ctx) {
        transformExprAfterTransformedBv2Int(
            ...
            (arg0 neq lowerBound) or (arg1 neq bv2IntContext.minusOne)
        }
    }
    ...
    override fun <T : KBvSort> transform(expr: KBvNegNoOverflowExpr<T>): KExpr<KBoolSort> = with(ctx) {
        transformExprAfterTransformedBv2Int(
            ...
            val (lowerBound, _) = getBounds(expr.value.sort.sizeBits, SIGNED)

            value neq lowerBound
        }
    }
...
//            mkAndNoSimplify(args)
    ...
    fun <T : KSort> KExpr<T>.preprocessArg(mode: WrapMode): KExpr<T> {
        if (this !is KBv2IntAuxExpr) return this

        return when (mode) {
            WrapMode.NONE -> this
            WrapMode.DENORMALIZED -> denormalized
            NORMALIZED_SIGNED -> normalized(SIGNED)
            NORMALIZED_UNSIGNED -> normalized(UNSIGNED)
        }.uncheckedCast()
    }

    fun KExpr<*>.postRewriteResult(mode: WrapMode, sort: KSort): KExpr<*> {
        ...
        return when (mode) {
            ...
            NORMALIZED_SIGNED -> KBv2IntAuxExprNormalized(
                normalized = this.uncheckedCast(),
                sizeBits = sort.sizeBits,
                normalizedSignedness = SIGNED
            )
            NORMALIZED_UNSIGNED -> KBv2IntAuxExprNormalized(
                normalized = this.uncheckedCast(),
                sizeBits = sort.sizeBits,
                normalizedSignedness = UNSIGNED
            )
    ...
    fun <T : KSort> KExpr<T>.distributeDependencies(arg: KExpr<*>): KExpr<T> = apply {
    ...
    fun <T : KSort> KExpr<T>.distributeDependencies(arg0: KExpr<*>, arg1: KExpr<*>): KExpr<T> = apply {
        addLemma(mergeLemmas(arg0.getLemma(), arg1.getLemma()))
        addBvAndLemma(mergeLemmas(arg0.getBvAndLemma(), arg1.getBvAndLemma()))
        addOverflowLemma(mergeLemmas(arg0.getOverflowLemma(), arg1.getOverflowLemma()))
        updatePowerOfTwoMaxArg(max(arg0.getPowerOfTwoMaxArg(), arg1.getPowerOfTwoMaxArg()))
    }

    fun <T : KSort> KExpr<T>.distributeDependencies(
        arg0: KExpr<*>,
        arg1: KExpr<*>,
        arg2: KExpr<*>
    ): KExpr<T> = apply {
        addLemma(mergeLemmas(arg0.getLemma(), arg1.getLemma(), arg2.getLemma()))
        addBvAndLemma(mergeLemmas(arg0.getBvAndLemma(), arg1.getBvAndLemma(), arg2.getBvAndLemma()))
        addOverflowLemma(mergeLemmas(arg0.getOverflowLemma(), arg1.getOverflowLemma(), arg2.getOverflowLemma()))
    ...
    fun <T : KSort> KExpr<T>.distributeDependencies(args: List<KExpr<*>>): KExpr<T> = apply {
        if (args.isEmpty()) return this

        addLemma(mergeLemmas(args) { it.getLemma() })
        addBvAndLemma(mergeLemmas(args) { it.getBvAndLemma() })
        addOverflowLemma(mergeLemmas(args) { it.getOverflowLemma() })
     ...
     *  */
    fun <T : KSort> KExpr<T>.addForOverflowCheck(sort: KSort, flag: Boolean): KExpr<T> = with(ctx) {
        val expr = this@addForOverflowCheck

        if (!config.isLazyOverflow || !flag || sort !is KBvSort) return expr
...
//        val signedness = if (expr.isNormalizedUnsigned) Signedness.UNSIGNED else Signedness.SIGNED
        ...
        sequenceOf(UNSIGNED, SIGNED).forEach { signedness ->
    ...
    inline fun <T : KSort> transformExprAfterTransformedBv2Int(
        expr: KExpr<T>,
    ...
    inline fun <T : KSort, B : KExpr<*>> transformExprAfterTransformedBv2Int(
        expr: KExpr<T>,
        dependency: KExpr<*>,
    ...
    @Suppress("LongParameterList")
    inline fun <T : KSort, B0 : KExpr<*>, B1 : KExpr<*>> transformExprAfterTransformedBv2Int(
        expr: KExpr<T>,
        dependency0: KExpr<*>,
        dependency1: KExpr<*>,
        preprocessMode: WrapMode = defaultWrapMode,
    ...
    @Suppress("LongParameterList")
    inline fun <T : KSort, B0 : KExpr<*>, B1 : KExpr<*>, B2 : KExpr<*>> transformExprAfterTransformedBv2Int(
        expr: KExpr<T>,
        dependency0: KExpr<*>,
        dependency1: KExpr<*>,
        dependency2: KExpr<*>,
    ...
    @Suppress("LongParameterList")
    inline fun <T : KSort, A : KSort> transformExprAfterTransformedBv2Int(
        expr: KExpr<T>,
    ...
    inline fun <In : KExpr<T>, Out : KExpr<T>, T : KSort, A : KSort> transformExprAfterTransformedBv2IntDefault(
        expr: In,
        dependencies: List<KExpr<A>>,
    ...
    fun <T : KSort> KExpr<T>.tryAddBoundLemmas(
        sort: KSort,
    ...
    fun <T : KSort> KExpr<T>.tryUnwrap(): KExpr<T> =
    ...
    inner class KBv2IntAuxExprLazySignedness(
        ...
        override val isNormalizedSigned: Boolean
            get() = if (normalizedSignedness == null) true else normalizedSignedness == SIGNED
        override val isNormalizedUnsigned: Boolean
            get() = if (normalizedSignedness == null) true else normalizedSignedness == UNSIGNED
    ...
    inner class KBv2IntAuxExprConst(
    ...
    inner class KBv2IntAuxExprZeroExtension(
        private val normalized: KExpr<KIntSort>,
        sizeBits: UInt,
    ...
    @Suppress("LongParameterList")
    inner class KBv2IntAuxExprExtract(
        override val denormalized: KExpr<KIntSort>,
        private val normalized: KExpr<KIntSort>,
        ...
        override val isNormalizedSigned: Boolean = normalizedSignedness == SIGNED
        override val isNormalizedUnsigned: Boolean = normalizedSignedness == UNSIGNED
    ...
    inner class KBv2IntAuxExprShl(
        ...
        override val isNormalizedSigned: Boolean
            get() = normalizedSignedness == SIGNED
        override val isNormalizedUnsigned: Boolean
            get() = normalizedSignedness == UNSIGNED
    ...
    inner class KBv2IntAuxExprAshr(
        ...
        override fun normalized(signedness: Signedness): KExpr<KIntSort> = with(ctx) {
            toSignedness(denormalized, sizeBits, SIGNED, signedness)
        }
    }

    inner class KBv2IntAuxExprLshr(
        normalized: KExpr<KIntSort>,
        originalExpr: KBv2IntAuxExpr,
        override val shift: Long,
        sizeBits: UInt,
    ...
    inner class KBv2IntAuxExprNormalized(
        normalized: KExpr<KIntSort>,
        sizeBits: UInt,
        private val normalizedSignedness: Signedness,
    ) : KBv2IntAuxExpr(normalized.ctx, sizeBits) {
    ...
    inner class KBv2IntAuxExprSingleOverflow(
        ...
        fun normalizedOp(op: (KExpr<KIntSort>) -> KExpr<KBoolSort>): KExpr<KBoolSort> = with(ctx) {
            val (lowerBound, upperBound) = getBounds(sizeBits, normalizedSignedness)
            val step = mkPowerOfTwoExpr(sizeBits)
    ...
    inner class KBv2IntAuxExprDenormalized(
        override val denormalized: KExpr<KIntSort>,
        sizeBits: UInt
    ) : KBv2IntAuxExpr(denormalized.ctx, sizeBits) {
...
//                .addOverflowLemma(overflowLemma)
    ...
    fun KContext.getBounds(
        ...
        val diff = if (boundSignedness == SIGNED) {
    ...
    fun KContext.toSignedness(
        ...
        when (signedness) {
            UNSIGNED -> toUnsigned(value, sizeBits, valueSignedness)
            SIGNED -> toSigned(value, sizeBits, valueSignedness)
        }

    fun KContext.normalizeExpr(expr: KExpr<KIntSort>, signedness: Signedness, sizeBits: UInt): KExpr<KIntSort> =
        toSignedness(expr mod mkPowerOfTwoExpr(sizeBits), sizeBits, UNSIGNED, signedness)
